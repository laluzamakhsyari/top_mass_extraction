...
def main():
    ...
    rebin=20
    data=[]
    data_compare=np.loadtxt("/scratch/work/zam/top_mass_extraction/mathematica/kdata_{}_{}_173,2_et2.txt" .format(varname[numb],rebin))
    ...
    for k in xrange(len(mass)):
        data.append(np.loadtxt("/scratch/work/zam/top_mass_extraction/mathematica/kdata_{}_{}_{}_et2.txt" .format(varname[numb], rebin, mass[k])))
    
        hists[numb].plot_combine(data[k], data_compare, label="LO, {} GeV".format(mass[k]))
        
    plt.legend()
    plt.savefig("rebin_{}_{}_et2.pdf" .format(rebin,hists[numb].title))
    plt.close()
